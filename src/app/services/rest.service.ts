...
export class RestService {
  ...
  getAuthenticationOptions(options?: any) {
    const headers = new HttpHeaders(
      {
  ...
  get<TOut>(url: string, options?: any): Promise<TOut> {
  ...
  getImage<TOut>(url: string, options?: any): Promise<TOut> {
  ...
  post<TOut>(url: string, arg: any, options?: any): Promise<TOut> {
    const opt = this.getAuthenticationOptions(options);
    if (!opt) {
      return Promise.reject('Auth Failed, Token Invalid or Missing');
    }
  ...
  postFormData<TOut>(url: string, arg: any, options?: any): Promise<TOut> {
    return this.http.post(url, arg, options).toPromise()
      .catch(e => this.handleError(e));
  }

  put<TOut>(url: string, arg: any, options?: any): Promise<TOut> {
    const opt = this.getAuthenticationOptions(options);
    if (!opt) {
      return Promise.reject('Auth Failed, Token Invalid or Missing');
    }
